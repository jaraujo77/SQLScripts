...
SELECT @TEXT_TOFIND = 'gdmnt'

DECLARE colCursor CURSOR FOR 
SELECT	TABLE_NAME,
		COLUMN_NAME
FROM	INFORMATION_SCHEMA.COLUMNS
WHERE	DATA_TYPE in ('text','ntext','char','nchar','varchar','nvarchar')
...
BEGIN
		...
		SELECT @cmd = 'SELECT @cnt = count(*) FROM [' + @TABLE_NAME + '] WHERE [' + @COLUMN_NAME + '] like ''%' + @TEXT_TOFIND + '%'''

		EXEC sp_executesql @cmd, @parameters, @cnt = @cnt output
		if (@cnt > 0)
		BEGIN
			PRINT 'Match Found IN ' + @TABLE_NAME + '.' + @COLUMN_NAME
			SELECT @cmd = 'SELECT * FROM [' + @TABLE_NAME + '] WHERE [' + @COLUMN_NAME + '] like ''%' + @TEXT_TOFIND + '%'''
